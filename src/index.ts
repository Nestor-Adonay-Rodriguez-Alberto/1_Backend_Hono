...
app.get('/', (c) => c.text('API operativa'))
...
app.post('/auth/login', async (c) => {
  ...
  if (!providedUser || !providedPassword) {
    return c.json({ ok: false, message: 'Usuario y contraseña son requeridos' }, 400)
  }
  ...
  return c.json({ ok: true, message: 'Inicio de sesión exitoso' })
})
...
app.post('/auth/login-basic', (c) => {
  ...
  if (!header.startsWith('Basic ')) {
    return c.json({ ok: false, message: 'Encabezado Authorization Basic requerido' }, 400)
  }
